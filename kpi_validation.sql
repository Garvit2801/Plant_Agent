...
-- KPI validation helpers (run in BigQuery Editor or bq CLI)

-- 1) Recent snapshots landed
SELECT
  COUNT(*) AS rows_10min,
  MIN(ts) AS min_ts,
  MAX(ts) AS max_ts
...
-- 3) Predict specific power using last observation
...
SELECT *
FROM ML.PREDICT(MODEL `my-plant-agent-123456.plant_ops.spower_reg`,
  (SELECT last_row.*)
);
